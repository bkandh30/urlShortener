...
router.post('/links', linkCreationLimiter, asyncHandler(async (req, res) => {
...
router.get('/links/:shortId', asyncHandler(async (req, res) => {
...
router.get('/links/:shortId/qr', asyncHandler(async (req, res) => {
    const { shortId } = req.params;
    if (!shortId) {
        return res.status(404).json({
            error: {
